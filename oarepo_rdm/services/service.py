...
def check_fully_overridden(
    ...
    def wrapper(cls: _T) -> _T:
        ...
        for name in dir(base_class):
            value = getattr(base_class, name, None)
            ...
            if this_class_value is value:
                raise TypeError(f"Method with name {name} is not overridden in {cls.__name__}.")
        return cls
...
class DelegationToSpecializedServiceMixin(InvenioService):
    ...
    attribute_on_base_service: str = (
        ""  # Used when the specialized service is a subservice of the base RDMRecordsService
    )
