...
def test_description_search(app, db, search_clear, identity_simple):
    ...
    assert results["links"]["self"] == "http://localhost/api/user/records?page=1&q=jej&size=10&sort=bestmatch"
    assert results["hits"]["hits"][0]["links"]["self"] == f"http://localhost/api/modelc/{rec_id}/draft"
